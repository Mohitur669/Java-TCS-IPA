...
public class MotelOrder {
    public static void main(String[] args) {
        ...
        int ans = totalNoOfRoomsBooked(motels, m);
        if (ans > 0) {
    ...
    public static int totalNoOfRoomsBooked(Motel[] motels, String m) {
        int ans = 0;

        for (Motel ml : motels) {
            if (ml.getCabFacility().equalsIgnoreCase(m) && ml.getNoOfRoomsBooked() > 5) {
                ans += ml.getNoOfRoomsBooked();
            }
        }
...
class Motel {
    ...
    private String motelName, dateOfBooking, cabFacility;
